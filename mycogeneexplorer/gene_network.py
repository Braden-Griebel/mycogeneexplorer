...
def load_cobra_model_from_file(model: (str | IO), file_type: str = None):
    """
    Loads a cobra model
...
def find_edges_from_model(model) -> Sequence[tuple]:
    ...
    for group, item in rxn_met_df.groupby("met"):
        for i, j in itertools.combinations(list(item["met"]), 2):
...
def find_directed_edges_from_model(model: cobra.core.model,
    ...
    for metabolite, df in rxn_met_df.groupby("met"):
        ...
        reversible_sink_pos_coef_df = reversible_df[
            (reversible_df["coefficient"] > 0) & (reversible_df["flux_min"] < 0)].drop(
            "flux_max").rename({"flux_min": "flux"}, axis=1)
        ...
        source_df = pd.concat([sources_irreversible_df,
                               ...
                               reversible_source_neg_coef_df], axis=1)
        sinks_df = pd.concat([sinks_irreversible_df,
                              reversible_sink_pos_coef_df,
                              reversible_sink_neg_coef_df], axis=1)
        for source, sink in itertools.product(source_df["rxn"], sinks_df["rxn"]):
            ...
            if reciprocal_weight:
                edge_weight_dict["weight"] = np.reciprocal(
                    (source_df.loc[source, "flux"] * source_df.loc[source, "coefficient"]) + np.power(10, -30))
            else:
                edge_weight_dict["weight"] = (source_df.loc[source, "flux"] * source_df.loc[source, "coefficient"])
...
def write_network(network: (nx.Graph | nx.DiGraph),
                  out_path: str,
                  ) -> None:
    """
...
def read_network(in_file: (str | IO), directed: bool = False)->(nx.Graph|nx.DiGraph):
    """
